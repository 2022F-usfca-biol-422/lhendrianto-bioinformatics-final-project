...
vcf_with_metadata %>%
  select(sample, Bases, Isolation_source) %>% unique() %>%
  knitr::kable(col.names = c("Sample",
                             "Bases",
                             "Isolation source"), row.names = FALSE,
               align = "ccl")
...
**Table 2**: Each sample, position, and the reference nucleotide and SNP
