...
This report will analyze the SNPs found in the variants of SARS-CoV-2. The SNPs were established to effect the transmissibility of the disease. As a result, we will examine which variant has increased transmissibility and what SNPs the variant contains. We will also compare the SNPs between different variants and refer back to the original strain. (Make past tense)
...
```{r plot-scattered}
# produce a scatter plot that visualizes the relative mobility over time
# using the subsetted state long data
scatter_plot <- ggplot(data = long_subset,
                       aes(x = lubridate::ymd(date),
                           y = relative_mobility)) +
  geom_point(color = "hot pink", size = 0.1, na.rm = TRUE) +
  geom_smooth(method = "lm", na.rm = TRUE) +
  labs(title = paste0(state_to_analyze,
                      "'s",
                     " relative mobility over time during COVID era"),
         x = "Date",
         y = "Relative mobility")
